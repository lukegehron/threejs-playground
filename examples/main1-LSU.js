	...
	pick(normalizedPosition, scene, camera) {
		...
		if (intersectedCircleObjects.length == 1) {
			...
			if (intersectedStickyObjects.length == 1) {
				...
				if (colorBy) {
					let myCurrentStickyEmotion = stickyData.emotion + "."
					if (checkedEmos.includes(myCurrentStickyEmotion)) {
						labelText.innerHTML = commentToShow.slice(0, -1);
					}
				} else {
					if (checkedDepts.includes(stickyData.dept)) {
						labelText.innerHTML = commentToShow.slice(0, -1);;
					}
				}
			} else {
			...
			if (intersectedObjects.length && mouseInCanvas) {
				...
				if (labelText.innerHTML == "") {
					let newName = intersectedObjects[0].object.name.replace(/_/g, " ")
					newName = newName.replace(/[0-9]/g, '')
					labelText.innerHTML = newName
...
function init() {
	...
	//CREATE THE 3 PANO GEOMETRY
	{
		//PANO 1
		{
			...
			const texture = new THREE.TextureLoader().load('textures/equirectangular/LSU-1_1.jpg');
			const material = new THREE.MeshBasicMaterial({ map: texture });

			pano1 = new THREE.Mesh(geometry1, material);
			pano1.rotateY(0.74)
			pano1.position.set(4.48, .23, 4.27) //x*0.1, z*0.1, y*-0.1
		...
		//PANO 2
		{
			...
			const texture = new THREE.TextureLoader().load('textures/equirectangular/LSU-2_1.jpg');
			const material = new THREE.MeshBasicMaterial({ map: texture });

			pano2 = new THREE.Mesh(geometry1, material);
			pano2.rotateY(-1.14)
			pano2.position.set(-4.23, 0.11, 1.79)
		...
		//PANO 3
		{
			...
			const texture = new THREE.TextureLoader().load('textures/equirectangular/LSU-3_1.jpg');
			const material = new THREE.MeshBasicMaterial({ map: texture });

			pano3 = new THREE.Mesh(geometry1, material);
			pano3.rotateY(3.2)
			pano3.position.set(-0.07, .21, -5.26)
	...
	circle = UTIL.drawCircles({ x: 4.48, y: -1.5, z: 4.27 }, 0)
	clickableCircles.push(circle)
	scene.add(circle)

	circle = UTIL.drawCircles({ x: -4.23, y: -1.5, z: 1.79 }, 1)
	clickableCircles.push(circle)
	scene.add(circle)

	circle = UTIL.drawCircles({ x: 0.07, y: -1.5, z: -5.26 }, 2) //-0.07, .21, -5.26
	clickableCircles.push(circle)
	scene.add(circle)


	//ADD LIGHT (PROBABLY NOT REQ'D)
	...
	loader.load('LSU1.glb', function (gltf) {
	...
	for (let l = 0; l < depts.length; l++) {
		var starCountRef = firebase.database().ref('/department-' + depts[l] + '/').once('value').then(function (snapshot) {
			//POORLY NAMED ARRAY - SHOULD BE ALL USER DATA
...
function update() {
		...
		for (let i = 0; i < stickyNoteArray.length; i++) {
			...
			if (interectionObjs.length > 0) {
				if (interectionObjs[0].object.userData.hasOwnProperty("emotion")) {
					...
					stickyNoteArray[i].material.opacity = 0.0;
	...
	if (currentCameraPosition == 0) {
		camera.position.set(4.48, .23, 4.27)
		...
		camera.position.set(-4.23, 0.11, 1.79)
		...
		camera.position.set(-0.07, .21, -5.26)
		pano1.position.y = 10000;
		pano2.position.y = 10000;
		pano3.position.y = 0;
